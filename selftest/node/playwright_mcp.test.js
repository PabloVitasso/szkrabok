...
const closeSession = () =>
  withTimeout(session.close({ sessionName: SESSION_ID }), 10_000, 'close session');

const getSnapshot = async () => {
  const res = await withTimeout(
    playwrightMcp.snapshot({ sessionName: SESSION_ID }),
    10_000,
    'snapshot'
  );
...
const clickRef = ref =>
  withTimeout(playwrightMcp.click({ sessionName: SESSION_ID, ref }), 10_000, 'click');
