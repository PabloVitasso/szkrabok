...
const spyOpen =
  calls =>
  async ({ sessionName, launchOptions }) => {
...
const spyClone =
  (calls, result = { cloneId: 'allegro#c91' }) =>
  async name => {
    calls.push({ name });
    return result;
  };
...
describe('EX-1 — clone mode', () => {
  test('EX-1.1 sessionOpen called with isClone:true', async () => {
    ...
    const deps = {
      ...base(),
      sessionOpen: spyOpen(calls),
      getSession: mockGetSession({ throwNth: 1 }),
    };
    ...
    await _run(
      {
        session: {
          name: 'allegro',
          mode: 'template',
          navigation: { policy: 'ifBlank', url: 'https://example.com' },
        },
        test: {},
      },
      deps
...
describe('EX-1 — templateConflict', () => {
  ...
  test('EX-1.22 close-first + template open → sessionClose before sessionOpen', async () => {
    ...
    const deps = {
      ...
      sessionClose: async ({ sessionName }) => {
        order.push(`close-${sessionName}`);
        return {};
      },
      sessionOpen: async ({ sessionName }) => {
        order.push(`open-${sessionName}`);
        return { sessionName };
      },
    };
    await _run(
...
describe('EX-1 — enforceLaunchOptionsMatch', () => {
  ...
  test('EX-1.25 enforceLaunchOptionsMatch false + mismatch → warn, continue', async () => {
    ...
    try {
      ...
      assert.ok(
        warns.some(m => m.includes('mismatch') || m.includes('launchOptions')),
