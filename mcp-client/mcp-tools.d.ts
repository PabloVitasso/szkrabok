...
export interface SessionHandle {
  ...
  open(args: {
    url?: string;
    launchOptions?: Record<string, unknown>; // Browser launch options — preset, stealth, headless, viewport, userAgent, locale, timezone, disableWebGL
  }): Promise<unknown>;
...
export interface BrowserHandle {
  ...
  run_test(args: {
    grep?: string; // Filter tests by name (regex)
    params?: Record<string, unknown>; // Key/value params passed as TEST_* env vars to the spec (e.g. {url:"https://..."} → TEST_URL)
    config?: string; // Config path relative to repo root. Defaults to playwright.config.js
    project?: string; // Playwright project name to run (e.g. "automation"). Runs all projects if omitted.
    files?: string[]; // File or directory paths passed as positional args to playwright test (e.g. ["automation/rebrowser-check.spec.js"] or ["automation/"]). Relative to repo root.
    keepOpen?: boolean; // After the test run, reconnect the session if the test subprocess invalidated the MCP context. Chrome stays alive; this restores the Playwright connection to it. Default false.
  }): Promise<unknown>;
  ...
  run_file(args: {
    path: string; // Absolute or relative path to an .mjs script file
    fn?: string; // Named export to call. Defaults to "default".
    args?: Record<string, unknown>; // Arguments passed as second parameter to the function
  }): Promise<unknown>;
