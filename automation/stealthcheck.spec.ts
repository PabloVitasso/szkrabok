...
test('stealthcheck', async ({ page }, testInfo) => {
  ...
  type CheckResult = { name: string; cls: string; value: string };
  const intoliResults: CheckResult[] = await page.evaluate((checks) => {
  ...
  const fpResults: CheckResult[] = await page.evaluate((checks) => {
    const results: CheckResult[] = [];
    document.querySelectorAll('tr').forEach(tr => {
      const tds = tr.querySelectorAll('td');
      if (tds.length < 2) return;
      ...
      results.push({ name, cls: statusTd.className.trim(), value: statusTd.textContent?.trim() ?? '' });
    });
    return results;
  }, FP_CHECKS);
  ...
  // ── attach result ─────────────────────────────────────────────────────────
  const result = {
    intoli:    { passed: intoliPassed.length,  failed: intoliFailures.length, warned: intoliWarnings.length, failures: intoliFailures },
