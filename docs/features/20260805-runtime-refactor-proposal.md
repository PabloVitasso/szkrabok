...
Proposal, verified against `origin/main` @ `9e90297` (post firefox-engine merge,
worktree `.claude/worktrees/main-latest`). Every item below was checked against
the real file/line — this is a filtered and corrected version of an earlier
unverified review. Items that didn't hold up (see "Rejected" section) are
removed. No implementation yet.
